 ...
 * mp_is_phat_city_time()    - test if there is time for non-essential processes
 */
...
void mp_planner_time_accounting() {
    ...
    if (!is_phat_city && _was_phat_city) {
        rpt_exception(STAT_GENERIC_EXCEPTION_REPORT, "exiting phat city");
    } else if (is_phat_city && !_was_phat_city) {
        rpt_exception(STAT_GENERIC_EXCEPTION_REPORT, "entering phat city");
    }
